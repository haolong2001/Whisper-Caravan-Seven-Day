...
The browser build does not require the backend in production. If `NEXT_PUBLIC_BACKEND_URL` is unset, the app uses deterministic frontend fallback and the full fixed-spine run remains playable.
...
- fixed fourteen-day starter spine only
- no 26-card dynamic event pool yet
